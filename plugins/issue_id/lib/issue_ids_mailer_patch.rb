...
module IssueIdsMailerPatch
    ...
    module InstanceMethods
        ...
        def issue_add_with_full_id(issue, to_users)
            message = issue_add_without_full_id(issue, to_users)
            message.subject = replace_with_full_id(message.subject) if @issue.support_issue_id?
            message
        end

        def issue_edit_with_full_id(journal, to_users)
            message = issue_edit_without_full_id(journal, to_users)
            message.subject = replace_with_full_id(message.subject) if @issue.support_issue_id?
            message
        end
